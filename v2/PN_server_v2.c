...
int main(int argc, char *argv[]){
    ...
    int nb; // Fork value
    int client_number = 0; // Number of client connected to the server
    ...
    while (1){
        start_loop: //Start initialisation for the game
            ...
            switch (nb = fork()) {
                case -1:
                    perror("La création du processus fils est impossible...\n");
                    exit(-1);
                ...
                default:
                    ...
                    client_number++;
                    printf("Il y a maintenant %d clients sur le serveur\n", client_number);
